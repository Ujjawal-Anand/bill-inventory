      ...
      <form>
        ...
        <div className={styles.lineItems}>
                      ...
                      <Draggable key={item.id} draggableId={item.id} index={i}>
                        {(provided, snapshot) => (
                          ...
                          >
                            <LineItem
                              style={{ color: 'red' }}
                              key={i + item.id} index={i} name={item.name} quantity={item.quantity} rate={item.rate}
                              {...functions}
                            />
                          </div>
                        )}
          ...
          <div className={styles.addItem}>
            <button type="button" onClick={addHandler}><AddCircleOutlineIcon size="1.25em" className={styles.addIcon} /> Add Item</button>
          </div>
